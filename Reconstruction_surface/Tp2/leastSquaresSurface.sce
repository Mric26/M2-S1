...
function c = mcPlan(x,y,z)
  ...
  n = size(x,1);                                                                            // nombre de ligne
  x2 = x.^2;                                                                                // calcul des differentes variables necessaires
  xy = x.*y;                                                                                //  |
  y2 = y.^2;                                                                                //  |
  xz = x.*z;                                                                                //  |
  yz = y.*z;                                                                                //  |
  b = matrix([sum(xz);sum(yz);sum(z)],3,1);                                                 // calcul du second membre
  A = matrix([sum(x2),sum(xy),sum(x);sum(xy),sum(y2),sum(y);sum(x),sum(y),n],3,3);          // creation de la matrice A
