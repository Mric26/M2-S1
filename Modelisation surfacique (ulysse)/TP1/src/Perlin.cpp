...
const unsigned char PerlinNoise::perm[512] = {
...
double  PerlinNoise::grad3( int hash, double x, double y , double z ) {
...
// 3D simplex noise
double PerlinNoise::snoise3(double x, double y, double z) {
...
double PerlinNoise::noise(const vec3& x,int octave,double persistency)
{
    double temp=0.0;
...
PerlinNoise::PerlinNoise(const glm::uint nb_octave, const float persistency):
...
double PerlinNoise::Eval(glm::vec3 p) const
{
    return noise(p, m_nb_octave, m_persistency);
}
